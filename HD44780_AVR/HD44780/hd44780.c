...
void lcd_command(unsigned char command) { lcd_send(command, 0); }
void lcd_write(unsigned char value) { lcd_send(value, 1); }

void lcd_send(unsigned char value, unsigned char mode) {
	if (mode) LCD_PORT = LCD_PORT | (1 << LCD_RS);
		else LCD_PORT = LCD_PORT & ~(1 << LCD_RS);
...
void lcd_blinking_enable(void) {
	lcd_displayparams |= LCD_BLINKON;
	lcd_command(LCD_DISPLAYCONTROL | lcd_displayparams);
}

void lcd_blinking_diasable(void) {
	lcd_displayparams &= ~LCD_BLINKON;
	lcd_command(LCD_DISPLAYCONTROL | lcd_displayparams);
}
...
void lcd_create_char(unsigned char location, unsigned char *charmap) {
	...
	for (int i = 0; i < 8; i++) lcd_write(charmap[i]);
...
void lcd_puts(char *puts) { for (char *it = puts; *it; it++) lcd_write(*it); }
